  ...
  return <section className="py-24 px-6 relative overflow-hidden">
      <div className="container mx-auto max-w-7xl">
        <AnimatedSection className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-16">
          <div>
            <motion.span className="text-secondary font-display font-bold text-sm tracking-wider" initial={{
            opacity: 0,
            x: -20
          }} whileInView={{
            opacity: 1,
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {projects.map((project, index) => <AnimatedSection key={project.id} delay={index * 0.1} direction={index % 2 === 0 ? 'left' : 'right'}>
              <motion.div className="group relative overflow-hidden rounded-3xl bg-card aspect-[4/3]" whileHover={{
            scale: 1.02,
            ...
            damping: 20
                ...
                {/* Corner accent */}
                <motion.div className="absolute top-4 right-4 w-12 h-12 border-2 border-foreground/20 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity" whileHover={{
              scale: 1.2,
              rotate: 45
            }}>
